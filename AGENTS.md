...
- For local-only, reversible git operations, present the plan but do not block on a separate confirmation. This covers: staging and committing with files and message already drafted; switching to the base branch on a clean worktree; `git pull --ff-only` on the base branch; `git branch -d` of a feature branch (safe because `-d` refuses unmerged branches); `git fetch --prune`. Stop and ask only when the merge status is unclear, the worktree is dirty, a destructive shortcut (e.g. `git branch -D`, `git reset --hard`) would be used, or the scope of a commit is genuinely ambiguous.
