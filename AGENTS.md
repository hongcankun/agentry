...
2. **Update `agentry.json`.** Reflect any added/removed/renamed component in the relevant plugin's `skills`/`agents`/`rules` arrays, and its `description`/`keywords` if scope changed.
