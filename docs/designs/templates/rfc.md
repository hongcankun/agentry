<!-- Agentry RFC template. Copy this file to ../<id>-<short-kebab-title>.md, assign the next ID from ../README.md, fill each section, and delete guidance in parentheses. Use this tier for large, breaking, hard-to-reverse, or cross-team changes. Recap the accepted Why briefly, then spend the length on the How. -->
...
(Restate the core of the accepted change request: the problem and the desired outcome. Link the request under **Answers** above. Recap, do not re-argue; whether to do the work is already decided. Include just enough for a reader to evaluate the design without opening the request.)
...
(The chosen approach, at a level of detail an implementer can build from and a reviewer can evaluate. Cover architecture, key components, data or interface changes, and how they fit the existing system. Include diagrams or interface sketches where they help.)
...
(What this change reaches: components, interfaces, data, consumers, or dependents it touches, and explicitly what it leaves untouched. This is reach and effect, not intent; the scope of aims lives in Motivation's non-goals.)

## Migration and Rollout

(How existing users or data move to the new design, and how it ships, such as a version release and deprecation window, feature flags, or a phased rollout. For anything breaking or stateful this is mandatory, not optional.)
...
(How the running design will be observed, both to confirm it achieved the accepted outcome and to operate it over time. Name the signals, such as logs, metrics, traces, diagnostics, or equivalents, that let maintainers or users see health and diagnose it. This is design-level "what to observe", not code testing or instrumentation wiring.)
