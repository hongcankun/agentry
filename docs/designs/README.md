...
- Start written designs from the local templates in [`templates/`](./templates/): use [`design-doc.md`](./templates/design-doc.md) for lightweight design docs and [`rfc.md`](./templates/rfc.md) for full RFCs.
...
- Keep `Status` current using one of: `Draft`, `In review`, `Accepted`, `Rejected`, or `Superseded`.
...
- Do not add templates to the index; the index is only for numbered proposals.
