...
1. Follow the `review-publishing` skill as the authoritative procedure, including its output contract.
...
4. Draft inline comments and/or a summary comment from existing findings. Default to `draft only` unless the current instruction clearly approves publishing.
